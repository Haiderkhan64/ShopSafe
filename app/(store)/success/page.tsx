...
import { Check, CreditCard, Download, Share2, ShoppingBag, Shield } from "lucide-react";
import { useRouter, useSearchParams } from "next/navigation";
import { useEffect, useState } from "react";
import { useAuth } from "@clerk/nextjs";

// Simple inline loader to avoid context issues
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-indigo-50/20">
      <div className="text-center">
        <div 
          ...
          style={{ 
            borderTopColor: '#574095',
            borderRightColor: '#6B46C1',
          }}
        />
...
export default function PaymentSuccess() {
  ...
  // Security: Redirect if no order number
  useEffect(() => {
  ...
  useEffect(() => {
    const clearCart = async () => {
      if (orderNumber && !isClearing && isSignedIn) {
        setIsClearing(true);
    ...
    clearCart();
  }, [orderNumber, clearBasket, isSignedIn, isClearing]);

  // Show simple loader while checking auth or client mounting
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-indigo-50/20 flex items-start justify-center p-4">
      <Card className="w-full max-w-md bg-white mt-5 lg:mt-24 shadow-2xl rounded-2xl border-0 overflow-hidden">
        ...
        <div className="pt-8 pb-6 px-6 text-center" style={{ background: 'linear-gradient(135deg, #F3E8FF 0%, #E9D5FF 100%)' }}>
          ...
          <div className="mx-auto mb-6 relative inline-block">
            ...
            >
              <div className="bg-white p-2 rounded-full">
                <Check className="h-10 w-10" style={{ color: '#6B46C1' }} strokeWidth={3} />
              </div>
            </div>
          </div>
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="p-4 rounded-xl border-2 relative overflow-hidden" style={{ borderColor: '#E9D5FF', background: 'linear-gradient(135deg, #FEFCE8 0%, #FEF3C7 100%)' }}>
            <div className="absolute top-0 right-0 w-20 h-20 opacity-10" style={{ background: 'linear-gradient(135deg, #FFD700 0%, #FCD34D 100%)' }} />
            <div className="flex items-center gap-3 mb-3 relative">
              <div className="p-2 rounded-lg shadow-md" style={{ backgroundColor: '#FCD34D' }}>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-lg transition-all hover:scale-[1.02]" style={{ backgroundColor: '#F9FAFB' }}>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg transition-all hover:scale-[1.02]" style={{ backgroundColor: '#F9FAFB' }}>
              ...
              <div className="flex items-center gap-2">
                <CreditCard className="h-4 w-4" style={{ color: '#6B46C1' }} />
                <span className="text-sm font-bold text-gray-900">Card Payment</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center justify-center gap-2 p-3 rounded-lg" style={{ backgroundColor: '#F3E8FF' }}>
              <Shield className="w-4 h-4" style={{ color: '#FFD700' }} />
              <span className="text-xs font-bold" style={{ color: '#6B46C1' }}>
                Secure
              </span>
            </div>
        ...
        <CardFooter className="p-6 bg-gradient-to-br from-gray-50 to-purple-50/20 flex flex-col sm:flex-row gap-3">
          <Button 
            variant="outline" 
            className="flex-1 flex items-center justify-center gap-2 border-2 transition-all hover:scale-[1.02]"
            style={{ 
              borderColor: '#E9D5FF',
              color: '#6B46C1'
            }}
